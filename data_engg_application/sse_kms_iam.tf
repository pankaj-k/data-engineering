...
resource "aws_kms_key_policy" "data_engg_kms_key_policy" {
  key_id = aws_kms_key.data_engg_kms_key.id
  policy = jsonencode({
    Id = "example"
    Statement = [
      {
        ...
        Principal = {
          AWS = [
            ...
            "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${aws_iam_role.data_engg_role.name}",
